    ...
    <div class="d-flex justify-content-between">
        <div class="flex-grow-1 pe-2">
            <div class="d-flex justify-content-between">
                ...
                <div class="flex-grow-1 me-2 mb-3 form-floating">
                    <select class="form-select" name="date" id="inputid">
                        <option {{ old('date', $filterByDate) === '' ? 'selected' : '' }} value="">
                            All
                        </option>
                        @foreach ($allDates as $date)
                            <option {{ old('date', $filterByDate) == $date ? 'selected' : '' }}
                                value="{{ $date }}">{{ $date }}
                            </option>
                        @endforeach
                    </select>
                    <label for="inputid" class="form-label">Date</label>
                </div>
                <div class="d-flex align-items-end">
                    <button type="submit" class="btn btn-primary mb-3 me-2 px-4 py-3 flex-grow-1"
                        name="filter">Filter</button>
                    <a href="{{ route('orders.admin') }}"
                        class="btn btn-secondary mb-3 py-3 px-4 flex-shrink-1">Reset</a>
                </div>
            </div>
        </div>
    </div>
